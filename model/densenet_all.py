from model.densenet_cifar import densenetcif40
...
#select rank for each layer
  def create_lr_compression_task(self,ratio,compress_state=True):

    if compress_state:
        compression_tasks = {}
        for i, (w_get, module_name) in enumerate([((lambda x=x: getattr(x, 'weight')), name) for name, x in self.model.named_modules() if ('compressible_conv' in name )or('compressible_fc' in name)]):#
            compression_tasks[module_name] \
            = RankSelection(module_name=module_name,ratio=ratio).compress(
            w_get())
  ...
  def compression_evaluate(self,ratio):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    pre_model = self.model
    state_dict = pre_model.state_dict()
    compressed_model = getattr(low_rank_model_def, self.name + '_compressed')(self.create_lr_compression_task(ratio=ratio,compress_state=True))
...
class densenet40(densenet_all):
  def __init__(self):
    super(densenet40, self).__init__("densenet40", densenetcif40(), 'densenet40')